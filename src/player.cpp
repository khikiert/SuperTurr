...
Sprite* turriball_gameover;
Sprite* turriball_shield;
Sprite* turrican_shield;

PlayerSprite turriball;
PlayerSprite turrican;
PlayerSprite turricanUpgrade;
...
Player::action(double frame_ratio)
{
  ...
  if(dying == DYING_NOT) 
    {
      ...
      // Don't accelerate Turri if he is running against a wall
      if (target.x != base.x)
        {
          if ((size == SMALL) && (!in_turriwater(0)) && (!on_turriwater())) {
          ...
          if (physic.get_velocity_y() < 0)
            {
              ...
              physic.set_velocity_y(0);
              if (size == BIG) {
                play_sound(sounds[SND_FALL_ON_GROUND], SOUND_CENTER_SPEAKER);
              }
              else if (size == SMALL) {
                play_sound(sounds[SND_FALL_ON_GROUND_SMALL], SOUND_CENTER_SPEAKER);
              }
            }
      ...
      if(jumped_in_solid)
        {
          if (isbrick(base.x, base.y) ||
            ...
            {
              World::current()->trygrabdiamond(base.x, base.y - 32,BOUNCE);
          ...
          if (isbrick(base.x+ 31, base.y) ||
            ...
            {
              World::current()->trygrabdiamond(base.x+ 31, base.y - 32,BOUNCE);
  ...
  if ((!on_ground()) && (on_turriwater())  && (!in_turriwater(11))) play_sound(sounds[SND_IN_WATER], SOUND_CENTER_SPEAKER);
      if (in_turriwater(0)) {
////          if (physic.get_velocity_y() < 0)
////            {
////              base.y = (int)(((int)base.y / 32) * 32);
//              if (!in_turriwater(-15)) {
//                physic.set_velocity_y(-0.3);
//                 under_water = false;
//              }
//              else if (in_turriwater(-5)) {
//                physic.set_velocity_y(0.3);
//                 under_water = true;
//              }
              if (in_turriwater(-15)) under_water = true;
              if (in_turriwater(-11) && !in_turriwater(-12)) under_water = false;
              if (!in_turriwater(-16)) {
// dryfowanie
                if (under_water && !in_turriwater(-16) && input.down != DOWN) physic.set_velocity_y(0.3);
                else if (!under_water && input.up != DOWN) physic.set_velocity_y(-0.3);
              }
    if (input.up == DOWN) physic.set_velocity_y(0.3);
    else if (input.down == DOWN) physic.set_velocity_y(-0.3);
    else if (in_turriwater(-16)) physic.set_velocity_y(-0.3);


//              if (in_turriwater(-15)) under_water = true;
//              else if (in_turriwater(-1) ) under_water = ;

              if (physic.get_velocity_x() > 0) physic.set_velocity_x(0.5);
              else if (physic.get_velocity_x() < 0) physic.set_velocity_x(-0.5);
              else physic.set_velocity_x(0);
//              physic.set_velocity_x(0.3);
//              if (size == BIG) {
//                play_sound(sounds[SND_FALL_ON_GROUND], SOUND_CENTER_SPEAKER);
//              }
//              else if (size == SMALL) {
...
//      else if (on_turriwater()) {
////              physic.set_velocity_y(-0.3);
//              if (physic.get_velocity_x() > 0) physic.set_velocity_x(0.5);
 //             else if (physic.get_velocity_x() < 0) physic.set_velocity_x(-0.5);
  //            else physic.set_velocity_x(0);
   //           if (dir == RIGHT) physic.set_velocity_x(0.5);
    //          else if (dir == LEFT) physic.set_velocity_x(-0.5);
//      }
    }
...
bool
Player::in_turriwater(int y)
{
  return ( isturriwater(base.x + base.width / 2, base.y + base.height / 2 + y));
//           isturriwater(base.x + 1, base.y + base.height) ||
//           isturriwater(base.x + base.width - 1, base.y + base.height)  );
}

//bool
//Player::half_in_turriwater2()
//{
//  return ( isturriwater(base.x + base.width / 2, base.y + base.height / 2 - 15));
////           isturriwater(base.x + 1, base.y + base.height) ||
////           isturriwater(base.x + base.width - 1, base.y + base.height)  );
//}

bool
Player::on_turriwater()
{
  if (dir == RIGHT) return ( isturriwater(base.x , base.y + base.height));
  if (dir == LEFT) return ( isturriwater(base.x + base.width , base.y + base.height));
//           isturriwater(base.x + 1, base.y + base.height) ||
//           isturriwater(base.x + base.width - 1, base.y + base.height)  );
}
...
Player::handle_horizontal_input()
{
  ...
  if (in_turriwater(0) || on_turriwater()) {
    if (dir == RIGHT) vx = 0.5;
    else if (dir == LEFT) vx = -0.5;
    if (dirsign == 0 && size != SMALL) vx = 0;
    ax = 0;
  }
...
//              if (under_water && input.down != DOWN) physic.set_velocity_y(0.3);
//              else if (!under_water && input.up != DOWN) physic.set_velocity_y(-0.3);
...
//    if (input.up == DOWN) physic.set_velocity_y(0.3);
//    if (input.down == DOWN) physic.set_velocity_y(-0.3);
//    else if (input.up == UP && in_turriwater(-5)) physic.set_velocity_y(-0.3);
...
Player::handle_input()
{
  ...
  if (input.fire == DOWN && input.old_fire == UP && got_coffee)
    {
      World::current()->add_bullet(base.x + 30, base.y + 10, physic.get_velocity_x(), dir);
      input.old_fire = DOWN;
    }
...
Player::grabdiamonds()
{
  ...
  if (!dying)
    {
      World::current()->trygrabdiamond(base.x, base.y, NO_BOUNCE);
      World::current()->trygrabdiamond(base.x+ 31, base.y, NO_BOUNCE);

      World::current()->trygrabdiamond(base.x, base.y + base.height, NO_BOUNCE);
      World::current()->trygrabdiamond(base.x+ 31, base.y + base.height, NO_BOUNCE);

      if(size == BIG)
        {
          World::current()->trygrabdiamond(base.x, base.y + base.height / 2, NO_BOUNCE);
          World::current()->trygrabdiamond(base.x+ 31, base.y + base.height / 2, NO_BOUNCE);
        }

    }

  /* Enough diamonds for a One-up? */
  if (player_status.diamonds >= DIAMONDS_LIFEUP)
    {
      player_status.diamonds = player_status.diamonds - DIAMONDS_LIFEUP;
  ...
  if (!safe_timer.started() || (global_frame_counter % 2) == 0)
    {
      if (dying == DYING_SQUISHED)
        {
          turriball_gameover->draw(base.x - scroll_x, base.y);
        }
      else
        {
          PlayerSprite* sprite;
          ...
          // Draw arm overlay graphics when Turri is holding something
          ...
          if (invincible_timer.started() &&
             (invincible_timer.get_left() > TURR_INVINCIBLE_TIME_WARNING || global_frame_counter % 3))
            {
              if (size == SMALL || duck)
                turriball_shield->draw(base.x - scroll_x, base.y);
              else
                turrican_shield->draw(base.x - scroll_x, base.y);
            }
        }
    }     
...
//  if (base.y > screen->h)
//  {
    play_sound(sounds[SND_FALL_OFF_SCREEN], SOUND_CENTER_SPEAKER);
//  }
//  else {
//    play_sound(sounds[SND_FALL_OFF_SCREEN], SOUND_CENTER_SPEAKER);
//  }
  ...
  if (mode == SHRINK && size == BIG)
    {
      ...
      safe_timer.start(TURR_SAFE_TIME);
    }
  else
    {
      physic.enable_gravity(true);
...
{
  /* Keep Turri in bounds: */
  if (base.x < 0)
    { // Lock Turri to the size of the level, so that he doesn't fall of
      // on the left side
      base.x = 0;
