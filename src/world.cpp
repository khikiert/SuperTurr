...
World::~World()
{
  ...
  for (std::vector<BouncyDiamond*>::iterator i = bouncy_diamonds.begin();
       i != bouncy_diamonds.end(); ++i)
    delete *i;
...
World::draw()
{
  ...
  for (y = 0; y < 15; ++y)
    {
      for (x = 0; x < 21; ++x)
        {
          Tile::draw(32*x - fmod(scroll_x, 32), y * 32,
  ...
  for (unsigned int i = 0; i < bouncy_diamonds.size(); ++i)
    bouncy_diamonds[i]->draw();
...
{
  turr.action(frame_ratio);
  turr.check_bounds(level->back_scrolling, (bool)level->hor_autoscroll_speed);
  scrolling(frame_ratio);

  /* Handle bouncy diamonds: */
  for (unsigned int i = 0; i < bouncy_diamonds.size(); i++)
    bouncy_diamonds[i]->action(frame_ratio);
...
void World::scrolling(double frame_ratio)
{
  ...
  int turr_pos_x = (int)(turr.base.x + (turr.base.width/2));

  if (level->back_scrolling || debug_mode)
  {
    if(turr.old_dir != turr.dir && level->back_scrolling)
      scrolling_timer.start(CHANGE_DIR_SCROLL_SPEED);

    if(scrolling_timer.check())
    {
      float final_scroll_x;
      if (turr.physic.get_velocity_x() > 0)
        final_scroll_x = turr_pos_x - (screen->w - X_SPACE);
      else if (turr.physic.get_velocity_x() < 0)
        final_scroll_x = turr_pos_x - X_SPACE;
      else
      {
        if (turr.dir == RIGHT)
          final_scroll_x = turr_pos_x - (screen->w - X_SPACE);
        else if (turr.dir == LEFT && level->back_scrolling)
          final_scroll_x = turr_pos_x - X_SPACE;
      }

      scroll_x +=   (final_scroll_x - scroll_x)
                  / (frame_ratio * (CHANGE_DIR_SCROLL_SPEED / 100))
                  + (turr.physic.get_velocity_x() * frame_ratio + turr.physic.get_acceleration_x() * frame_ratio * frame_ratio);
      // std::cerr << turr_pos_x << " " << final_scroll_x << " " << scroll_x << std::endl;

    }
    else
    {
      if (turr.physic.get_velocity_x() > 0 && scroll_x < turr_pos_x - (screen->w - X_SPACE))
        scroll_x = turr_pos_x - (screen->w - X_SPACE);
      else if (turr.physic.get_velocity_x() < 0 && scroll_x > turr_pos_x - X_SPACE && level->back_scrolling)
        scroll_x = turr_pos_x - X_SPACE;
      else
      {
        if (turr.dir == RIGHT && scroll_x < turr_pos_x - (screen->w - X_SPACE))
            scroll_x = turr_pos_x - (screen->w - X_SPACE);
        else if (turr.dir == LEFT && scroll_x > turr_pos_x - X_SPACE && level->back_scrolling)
            scroll_x = turr_pos_x - X_SPACE;
      }
    }
  }
...
World::collision_handler()
{
  ...
  if(turr.dying != DYING_NOT) return;
  ...
  for (BadGuys::iterator i = bad_guys.begin(); i != bad_guys.end(); ++i)
    {
      ...
      if(rectcollision_offset((*i)->base, turr.base, 0, 0))
        {
          // We have detected a collision and now call the collision
          // functions of the collided objects.
          if (turr.previous_base.y < turr.base.y &&
              turr.previous_base.y + turr.previous_base.height 
              < (*i)->base.y + (*i)->base.height/2
              && !turr.invincible_timer.started())
            {
              (*i)->collision(&turr, CO_PLAYER, COLLISION_SQUISH);
            }
          else
            {
              turr.collision(*i, CO_BADGUY);
              (*i)->collision(&turr, CO_PLAYER, COLLISION_NORMAL);
            }
        }
    }
  ...
  for(unsigned int i = 0; i < upgrades.size(); ++i)
    {
      if(rectcollision(upgrades[i].base, turr.base))
        {
          // We have detected a collision and now call the collision
          // functions of the collided objects.
          upgrades[i].collision(&turr, CO_PLAYER, COLLISION_NORMAL);
        }
    }
}
...
void
World::add_bouncy_diamond(float x, float y)
{
  BouncyDiamond* new_bouncy_diamond = new BouncyDiamond();
  new_bouncy_diamond->init(x,y);
  bouncy_diamonds.push_back(new_bouncy_diamond);
}
...
World::play_music(int musictype)
{
  ...
  switch(currentmusic) {
    case HURRYUP_MUSIC:
      ...
      break;
    case LEVEL_MUSIC:
//      std::cout << "play_music() from world.cpp :: case LEVEL_MUSIC" << "\n";
      music_manager->play_music(get_level()->get_level_music(), level->music_loops);
      break;
//OSLONKA    case HERRING_MUSIC:
//OSLONKA      music_manager->play_music(herring_song);
//OSLONKA      break;
    default:
  ...
  if (tile->brick)
    {
      if (tile->data > 0)
        {
          /* Get a diamond from it: */
          add_bouncy_diamond(((int)(x + 1) / 32) * 32,
                                  (int)(y / 32) * 32);

          // TODO: don't handle this in a global way but per-tile...
          if (!counting_diamonds)
            {
              counting_diamonds = true;
              diamond_counter = 5;
            }
          else
            {
          ...
          play_sound(sounds[SND_DIAMOND], SOUND_CENTER_SPEAKER);
          player_status.score = player_status.score + SCORE_DIAMOND;
          player_status.diamonds++;
        }
      else if (!small)
        {
          /* Get rid of it: */
...
World::tryemptybox(float x, float y, Direction col_side)
{
  ...
  switch(tile->data)
    {
    case 1: // Box with a diamond!
      add_bouncy_diamond(posx, posy);
      play_sound(sounds[SND_DIAMOND], SOUND_CENTER_SPEAKER);
      player_status.score = player_status.score + SCORE_DIAMOND;
      player_status.diamonds++;
      break;
    ...
    case 2: // Add an upgrade!
      if (turr.size == SMALL)     /* Turri is small, add mints! */
        add_upgrade(posx, posy, col_side, UPGRADE_GROWUP);
      else     /* Turri is big, add an iceflower: */
...
/* Try to grab a diamond: */
void
World::trygrabdiamond(float x, float y, int bounciness)
{
  Tile* tile = gettile(x, y);
  ...
  for (BadGuys::iterator i = bad_guys.begin(); i != bad_guys.end(); ++i)
    {
      if ((*i)->base.x >= x - 32 && (*i)->base.x <= x + 32 &&
        ...
        {
          (*i)->collision(&turr, CO_PLAYER, COLLISION_BUMP);
        }
    }
  ...
  for (unsigned int i = 0; i < upgrades.size(); i++)
    {
      if (upgrades[i].base.height == 32 &&
        ...
        {
          upgrades[i].collision(&turr, CO_PLAYER, COLLISION_BUMP);
        }
    }
}
