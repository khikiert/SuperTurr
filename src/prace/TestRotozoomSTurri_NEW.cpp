...
void TestRotozoomSTurri::RotatePicture(SDL_Surface * screen, Surface * picture, int rotate, int flip, int smooth, int position, Surface * background, bool show_background) 
{
...
//    SDL_UpdateRect(screen, 0, 0, background->w, background->h);
	...
	for (framecount = 0, frameinc = 1; framecount < framemax; framecount += frameinc) {
		...
		HandleEvent();
//		clearscreen(0, 0, 0);
		if (use_gl)	ants_pointer.at(framecount)->draw_stretched((screen->w - rotozoom_pictureSDL[framecount]->w) / 2,
																	30,
...
updatescreen();
		...
		/* Delay */
		SDL_Delay(delay + (framecount / 104) * (framecount / 208) * 5);
	}
...
/*#if SDL_BYTEORDER == SDL_BIG_ENDIAN
  ...
  background_img = new Surface(datadir + "/images/background/superturr/title.png", USE_ALPHA);
	...
	title_animation = 1;
	RotatePicture(screen, title_img, 0, 0, 0, 1, background_img, true);
