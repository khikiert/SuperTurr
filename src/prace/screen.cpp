...
//SDL_to_GL * ants_GL[256];
SDL_to_GL * ants;
std::vector<SDL_to_GL *> ants_GL(256);
//		SDL_to_GL * ants_GL[256];
...
ants_GL.at(alphadiff - 1) = new SDL_to_GL(ants_SDL[alphadiff - 1]);
//ants_GL.at(1) = new SDL_to_GL();
//ants_GL[1] = new SDL_to_GL();
...
if (fade_out)
  ...
  alpha = 255;
	...
	dest.h = screen->h;
//alpha = 1;
if (use_gl) ants_GL.at(0)->show_image(alpha);
  while(alpha >= 0 && alpha < 256)
    {
clearscreen(0, 0, 0);
...
//	glEnd();
//	ants_GL.at(1)->SDL_GL_Leave2DMode();
//ants->show_image();
ants_GL.at(0)->show_image(alpha);
		}
			else {
