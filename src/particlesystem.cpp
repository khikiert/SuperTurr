...
void ParticleSystem::draw(float scrollx, float scrolly, int layer)
{
    ...
    for(i = particles.begin(); i != particles.end(); ++i) {
        ...
        float x = fmod(particle->x - scrollx, virtual_width);
        if(x < 0) x += virtual_width;
        float y = fmod(particle->y - scrolly, virtual_height);
        if(y < 0) y += virtual_height;
        float xmax = fmod(x + particle->texture->w, virtual_width);
        float ymax = fmod(y + particle->texture->h, virtual_height);
...
void SnowParticleSystem::simulate(float elapsed_time)
{
    ...
    for(i = particles.begin(); i != particles.end(); ++i) {
        ...
        if(particle->y > screen->h) {
            particle->y = fmod(particle->y , virtual_height);
            particle->x = rand() % int(virtual_width);
        }
    }
}
