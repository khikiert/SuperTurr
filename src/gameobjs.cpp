...
void
BouncyDiamond::init(float x, float y)
{
  base.x = x;
  base.y = y;
  base.ym = -2;
...
void
BouncyDiamond::action(double frame_ratio)
{
  ...
  if (base.ym >= 0)
    {
      std::vector<BouncyDiamond*>::iterator i
        = std::find(World::current()->bouncy_diamonds.begin(), 
                    World::current()->bouncy_diamonds.end(), 
                    this);
      if (i != World::current()->bouncy_diamonds.end())
        World::current()->bouncy_diamonds.erase(i);
    }
}
...
BouncyDiamond::draw()
{
  img_diamond[0]->draw(base.x - scroll_x,
                      base.y);
}
