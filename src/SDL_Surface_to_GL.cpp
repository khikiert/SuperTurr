...
//	glBlendFunc(GL_SRC_ALPHA, GL_DST_ALPHA);		//rozjasnianie od aktywnego obrazu
//	glBlendFunc(GL_SRC_ALPHA, GL_ZERO);		//rozjasnianie od zera
//	glBlendFunc(GL_SRC_ALPHA, GL_ONE);		//rozjasnianie od aktywnego obrazu
//	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_CONSTANT_ALPHA);		//rozjasnianie od aktywnego obrazu
//	glBlendFunc(GL_SRC_ALPHA, GL_CONSTANT_COLOR);		//rozjasnianie od zera
//	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_CONSTANT_COLOR);		//rozjasnianie od aktywnego obrazu
//	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_DST_ALPHA);		//rozjasnianie od zera
//	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_DST_COLOR);		//najpierw obraz robic sie lekko widoczny i ciemnieje, a potem rozjasnia sie do poziomu nieco ponad pierwotny
// alpha = 200;
int x = 255;
//	glColor4ub(alpha, alpha, alpha, alpha);
// 			alpha : 255 - zerowa przezroczystosc warstwy
// 					0   - pelna przezroczystosc warstwy
	glColor4ub(x, x, x, alpha);
}
...
void SDL_Surface_to_GL::display_image(SDL_Surface_to_GL * object, int alpha) {
	/* Show the image on the screen */
	SDL_GL_Enter2DMode(alpha);
	
	glBindTexture(GL_TEXTURE_2D, global_texture.texture);	
...
void SDL_Surface_to_GL::create_GL_tex_from_file(char * png_file) {
	if (!global_texture.texture) {
		...
		global_texture.texture = SDL_GL_LoadTexture(sdl_surface, texcoord);
		...
		if (!global_texture.texture) return;
	}
}
...
void SDL_Surface_to_GL::create_GL_tex_from_display() {
	SDL_Surface * sdl_surface;
