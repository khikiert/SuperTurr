...
#ifndef SUPERTURR_WORLD_H
#define SUPERTURR_WORLD_H
...
/** The World class holds a level and all the game objects (badguys,
    bouncy diamonds, etc) that are needed to run a game. */
class World
{
private:
  ...
  int diamond_counter;
  bool counting_diamonds;
  int currentmusic;

  static World* current_;
public:
  BadGuys bad_guys;
  std::vector<BouncyDiamond*> bouncy_diamonds;
...
public:
  ...
  void add_bouncy_diamond(float x, float y);
  ...
  void trygrabdiamond(float x, float y, int bounciness);
  ...
  /** Try to bumb a badguy that might we walking above Turri, thus shaking
