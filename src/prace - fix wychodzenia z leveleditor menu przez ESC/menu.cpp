...
Menu::set_current(Menu* menu)
{
  ...
  if (menu) 
    ...
    else if (menu == worldmap_menu) {
    	menu->effect.start(500);
    }
	else if (title_animation) {
		title_animation = 0;
		menu->effect.start(500);
	}
    else if (current_ != main_menu) {
    	menu->effect.start(750);
    }
  current_ = menu;
  fprintf(stdout, "AKTYWNE MENU OBECNIE: %i\n", current_);
}
...
Menu::Menu()
{
  ...
  menu_timer.init(true);
  menu_timer.start(750);
}
...
Menu::event(SDL_Event& event)
{
  ...
  case SDL_KEYDOWN:
    ...
    if(item[active_item].kind == MN_CONTROLFIELD)
    {
      if(key == SDLK_ESCAPE)
      {
fprintf(stdout, "KURWAAAAAAAAAAAAA!!!!!!!!!!!!!!! \n");
        Menu::pop_current();
        return;
      }
    ...
    case SDLK_RETURN: /* Menu Hit */
      ...
      play_sound(sounds[SND_MENU_SELECT_OPT], SOUND_CENTER_SPEAKER);
      break;
    case SDLK_DELETE:
    case SDLK_BACKSPACE:
      ...
      break;
    case SDLK_ESCAPE: {
Menu * menuhuj = Menu::current();
fprintf(stdout, "KURWAAAAAAAAAAAAA!!!!!!!!!!!!!!!222222222222222222222222 %i %i %i\n", Menu::current()->check(), Menu::current(), le_menu_check);
//if (menuhuj = Leveleditor::leveleditor_menu) fprintf(stdout, "I TO JEST KURWA LEVELEDITOR!!! \n");
if ((Menu::current() == get_le_menu()) && (le_level_loaded != 1)) {
		quit_level_editor = 1;
//	le_quit();
//	return;
}
...
//return;
      Menu::pop_current();
  //    Menu * menuhuj2 = Menu::current();
//if (menuhuj == menuhuj2) Menu::pop_current();
//le_quit();
//      done = 1;
      }
      break;
