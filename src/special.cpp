...
Bullet::action(double frame_ratio)
{
  ...
  if (issolid(base.x, base.y + 4) || issolid(base.x, base.y))
    {
// strzal odbija sie od ziemi
		  play_sound(sounds[SND_BULLET_BOUNCING], SOUND_CENTER_SPEAKER);
      base.y  = old_y;
      base.ym = -base.ym;     
      if (base.ym > 9)
        base.ym = 9;
...
Bullet::draw()
{
  if (base.x >= scroll_x - base.width &&
      base.x <= scroll_x + screen->w)
    {
      img_bounceshot->draw(base.x - scroll_x, base.y);
    }
}
  ...
  if (base.height < 32)
    {
      ...
      if (kind == UPGRADE_GROWUP)
        img_mode1->draw_part(0,0,dest.x,dest.y,dest.w,dest.h);
      else if (kind == UPGRADE_ICEFLOWER)
        img_mode2->draw_part(0,0,dest.x,dest.y,dest.w,dest.h);
      else if (kind == UPGRADE_HERRING)
        img_shield->draw_part(0,0,dest.x,dest.y,dest.w,dest.h);
    ...
    case CO_PLAYER:
      ...
      if (kind == UPGRADE_GROWUP)
      ...
      else if (kind == UPGRADE_HERRING)
        {
          ...
          pplayer->invincible_timer.start(TURR_INVINCIBLE_TIME);
//OSLONKA          World::current()->play_music(HERRING_MUSIC);
        }
      else if (kind == UPGRADE_1UP)
        {
...
{
  img_mode1    = sprite_manager->load("bonus-mode1");
  img_mode2 = sprite_manager->load("bonus-mode2");
  img_shield      = sprite_manager->load("bonus-shield");
  img_1up       = sprite_manager->load("bonus-1up");

  img_bounceshot    = sprite_manager->load("bounceshot");
}
