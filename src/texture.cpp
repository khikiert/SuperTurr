...
Surface* Surface::CaptureScreen()
{
	...
	if (use_gl)
	{
		...
		temp = SDL_CreateRGBSurface(SDL_SWSURFACE, screen->w, screen->h, 24, Rmask, Gmask, Bmask, Amask);

		if (temp == NULL) st_abort("Error while trying to capture the screen in OpenGL mode","");
...
void SurfaceImpl::set_sdl_surface(SDL_Surface * surf) {
    sdl_surface = sdl_surface_from_sdl_surface(surf, 1);
...
SurfaceSDL::draw(float x, float y, Uint8 alpha, bool update)
{
  ...
  if(alpha != 255)
    ...
    if (update == UPDATE) SDL_UpdateRect(screen, dest.x, dest.y, dest.w, dest.h);
