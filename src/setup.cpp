...
#ifdef __MORPHOS__
#define DATA_PREFIX "PROGDIR:data"
...
/* Makes sure a directory is created in either the Super Turr home directory or the Super Turr base directory.*/
...
/* --- SETUP --- */
/* Set Super Turr configuration and save directories */
...
//  st_dir = (char *) malloc(sizeof(char) * ( strlen(home) + strlen("/.superturr") + 1) );
//  strcpy(st_dir, home);
//  strcat(st_dir, "/.superturr");
#ifdef __MORPHOS__
  st_dir = (char *) malloc(sizeof(char) * ( strlen("PROGDIR:.superturr") + 1) );
  strcpy(st_dir, "PROGDIR:.superturr");
#elif _WIN32
  ...
  /* Remove .superturr config-file from old Super Turr versions */
...
#ifdef __MORPHOS__
	std::string exedir = std::string("PROGDIR:");
	datadir = DATA_PREFIX; // Super Turr run from source dir
	if (access(datadir.c_str(), F_OK) != 0) {
		datadir = DATA_PREFIX; // Super Turr run from PATH
  ...
  datadir = DATA_PREFIX;
      ...
      if (readlink("/proc/self/exe", exe_file, PATH_MAX) < 0)
      ...
      else
        {
          ...
          datadir = exedir + "../data"; // Super Turr run from source dir
          if (access(datadir.c_str(), F_OK) != 0)
            {
              datadir = exedir + "../share/superturr"; // Super Turr run from PATH
              if (access(datadir.c_str(), F_OK) != 0) 
...
bool process_load_game_menu()
{
  ...
  if(slot != -1 && load_game_menu->get_item_by_id(slot).kind == MN_ACTION)
    {
//  play_sound(sounds[SND_MENU_SELECT_OPT2], SOUND_CENTER_SPEAKER);
      char slotfile[1024];
      snprintf(slotfile, 1024, "%s/slot%d.stsg", st_save_dir, slot);

      if (access(slotfile, F_OK) != 0)
        {
// wyswietlenie intro - odegraj muze INTRO.OGG
//					music_manager = new MusicManager();
//					menu_song = music_manager->load_music(datadir + "music/credits.ogg");
//					music_manager->halt_music();
//					music_manager->play_music(intro_music,0);
//					music_manager = new MusicManager();
                    MusicRef intro_music = music_manager->load_music(datadir + "/music/superturr/intro_III.ogg");
					music_manager->halt_music();
					music_manager->play_music(intro_music,-1);
          draw_intro();
		player_status.lives = 4;
        }
...
void st_general_setup(void)
{
  ...
  gold_text   = new Text(datadir + "/images/status/superturr/turrican3_fonts_yellow.png", TEXT_TEXT, 16,14);
  silver_text = new Text(datadir + "/images/status/letters-silver.png", TEXT_TEXT, 16,18);
  blue_text   = new Text(datadir + "/images/status/superturr/turrican3_fonts_red.png", TEXT_TEXT, 16,14);
  red_text    = new Text(datadir + "/images/status/superturr/turrican3_fonts_red.png", TEXT_TEXT, 16,14);
  green_text  = new Text(datadir + "/images/status/superturr/turrican3_fonts_green.png", TEXT_TEXT, 16,18);
  white_text  = new Text(datadir + "/images/status/superturr/turrican3_fonts_white.png", TEXT_TEXT, 16,14);
  white_small_text = new Text(datadir + "/images/status/letters-white-small.png", TEXT_TEXT, 8,9);
  white_big_text   = new Text(datadir + "/images/status/superturr/turrican3_fonts_white.png", TEXT_TEXT, 16,14);
  yellow_nums = new Text(datadir + "/images/status/numbers.png", TEXT_NUM, 32,32);
//  turri_text1   = new Text(datadir + "/images/status/turri-lives_letters.png", TEXT_TEXT, 16,32);
  lives_text   = new Text(datadir + "/images/status/superturr/superturrican_fonts_lives.png", TEXT_TEXT, 21,27);
  ...
  mouse_cursor = new MouseCursor( datadir + "/images/status/superturr/mousecursor.png",1);
...
blit_fill: %u \n\
video_mem: %u\n",
videoinf->hw_available,
videoinf->wm_available,
videoinf->blit_hw,
videoinf->blit_hw_CC,
videoinf->blit_hw_A,
videoinf->blit_sw,
...
videoinf->video_mem);
  ...
  SDL_WM_SetCaption("Super Turr " VERSION, "Super Turr");
}
      ...
      if (strcmp(argv[i], "--fullscreen") == 0 ||
        ...
        {     /* Show help: */
          ...
          puts("Display Options:\n"
               ...
               "  --version           Display the version of Super Turr you're running.\n\n"
               );
          exit(0);
        }
      else if (argv[i][0] != '-')
