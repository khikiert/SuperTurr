...
{
  std::string stwt_filename = datadir +  "/images/worldmap/walkers_island.stwt";
...
Turr::Turr(WorldMap* worldmap_)
  : worldmap(worldmap_)
{
  turrican_sprite = new Surface(datadir +  "/images/worldmap/superturr/turr.png", USE_ALPHA);
  turricanUpgrade_sprite = new Surface(datadir +  "/images/worldmap/superturr/turr.png", USE_ALPHA);
  turriball_sprite = new Surface(datadir +  "/images/worldmap/superturr/turr.png", USE_ALPHA);
...
Turr::~Turr()
{
  delete turriball_sprite;
  delete turricanUpgrade_sprite;
  delete turrican_sprite;
}

void
Turr::draw(const Point& offset)
{
  Point pos = get_pos();
  switch (player_status.bonus)
    {
    case PlayerStatus::GROWUP_BONUS:
      turrican_sprite->draw(pos.x + offset.x, 
                            pos.y + offset.y - 10);
      break;
    case PlayerStatus::FLOWER_BONUS:
      turricanUpgrade_sprite->draw(pos.x + offset.x, 
                           pos.y + offset.y - 10);
      break;
    case PlayerStatus::NO_BONUS:
      turriball_sprite->draw(pos.x + offset.x, 
                            pos.y + offset.y - 10);
      break;
    }
}
...
Turr::update(float delta)
{
  if (!moving)
    ...
    {
      // Let Turri walk a few pixels (20 pixel/sec)
      offset += 20.0f * delta;
...
{
  if (enter_level && !turr->is_moving())
    {
      Level* level = at_level();
      if (level && !level->name.empty())
        {
          if (level->x == turr->get_tile_pos().x && 
              level->y == turr->get_tile_pos().y)
            {
                ...
                case GameSession::ES_LEVEL_FINISHED:
                  {
                    ...
                    if (session.get_world()->get_turr()->got_coffee)
                      player_status.bonus = PlayerStatus::FLOWER_BONUS;
                    else if (session.get_world()->get_turr()->size == BIG)
                    ...
                    if (old_level_state != level->solved && level->auto_path)
                      { // Try to detect the next direction to which we should walk
                        ...
                        Tile* tile = at(turr->get_tile_pos());

                        if (tile->north && turr->back_direction != D_NORTH)
                          dir = D_NORTH;
                        else if (tile->south && turr->back_direction != D_SOUTH)
                          dir = D_SOUTH;
                        else if (tile->east && turr->back_direction != D_EAST)
                          dir = D_EAST;
                        else if (tile->west && turr->back_direction != D_WEST)
                          dir = D_WEST;

                        if (dir != D_NONE)
                          {
                            turr->set_direction(dir);
                            //turr->update(delta);
                          }
...
fprintf(stdout, "wybrano abort level. map fade in = %i\n", this->map_fade_in);
					this->map_fade_in = 2;
fprintf(stdout, "wybrano abort level. map fade in po zmianie = %i\n", this->map_fade_in);
                ...
                case GameSession::ES_GAME_OVER:
                  quit = true;
                  player_status.reset();
				// ponizsze dodane, zeby nie wyswietlal sie fade out na mape
              ...
              level->y == turr->get_tile_pos().y)
				{
					...
					turr->back_direction = D_NONE;
					turr->set_tile_pos(Point(level->teleport_dest_x, level->teleport_dest_y));
					SDL_Delay(1000);
				}
		}
		else
        {
          std::cout << "Nothing to enter at: "
                    << turr->get_tile_pos().x << ", " << turr->get_tile_pos().y << std::endl;
        }
    }
  else
    {
      turr->update(delta);
      turr->set_direction(input_direction);
    }
...
WorldMap::at_level()
{
  for(Levels::iterator i = levels.begin(); i != levels.end(); ++i)
    {
      if (i->x == turr->get_tile_pos().x && 
          i->y == turr->get_tile_pos().y)
        return &*i; 
    }
...
WorldMap::draw_status()
{
  ...
  white_text->draw("SCORE", 4, 4);
  red_text->draw(str, 4, 22);

  sprintf(str, "%d", player_status.diamonds);
////  white_text->draw_align("DIAMONDS", 320-64, 0,  A_LEFT, A_TOP);
////  gold_text->draw_align(str, 320+64, 0, A_RIGHT, A_TOP);
  white_text->draw("DIAMONDS", 508, 4, 1);
//  if (str == "1" || str == "2" || str == "3" || str == "4" || str == "5" || str == "6" || str == "7" || str == "8" || str == "9" || str == "0") red_text->draw(str, 622, 22, 1);
      ...
      for(Levels::iterator i = levels.begin(); i != levels.end(); ++i)
        {
          if (i->x == turr->get_tile_pos().x && 
              i->y == turr->get_tile_pos().y)
            {
              if(!i->name.empty())
                {
              white_text->draw_align(i->title.c_str(), screen->w/2, screen->h - 5,  A_HMIDDLE, A_BOTTOM);
                }
...
//	fade_screen(15, true);
//    fade_screen(15, "fadein", 4);      
    

// jesli map_fade_in != 1, najpierw nastapi fade in na mape
this->map_fade_in = 0;
  ...
  while(!quit)
    {
      ...
      Point turr_pos = turr->get_pos();
      if (1)
        {
          offset.x = -turr_pos.x + screen->w/2;
          offset.y = -turr_pos.y + screen->h/2;
	...
	// fade in na widok mapy
	if (this->map_fade_in != 1) {
fprintf(stdout, "map fade in w petli zmienil sie i wynosifff = %i\n", this->map_fade_in);
//		if (map_fade_in == 2) { clearscreen(100, 200, 0); flipscreen();
//		SDL_Delay(1000);
//		}
//	while (this->map_fade_in == 2) { }
//flipscreen();

		SDL_Surface * map_display;
		if (use_gl) {					// OpenGL ON
			SDL_Surface * scr_cap;
			flipscreen();
			scr_cap = Surface::CaptureScreen()->impl->get_sdl_surface();
			map_display = SDL_DisplayFormat(scr_cap);
		}
		else {							// OpenGL OFF
			SDL_Surface * scr_cap;
//			flipscreen();
			scr_cap = Surface::CaptureScreen()->impl->get_sdl_surface();
			map_display = SDL_DisplayFormat(scr_cap);
//			map_display = SDL_DisplayFormatAlpha(screen);
		}
		clearscreen(0, 0, 0);
		if (this->map_fade_in == 0) fade_surface(map_display, 15, "fadein", 4, true);
//fade_screen(15, "fadein", 4, true);
		this->map_fade_in  = 1;
fprintf(stdout, "...a teraz juz = %i\n", this->map_fade_in);
	}

      SDL_Delay(20);
    }

	// fade out po wyjsciu z mapy do menu glownego
fprintf(stdout, "exit status: lives: %i", player_status.lives);
...
WorldMap::savegame(const std::string& filename)
{
  ...
  out << "(supertux-savegame\n"
      << "  (version 1)\n"
      << "  (title  \"Walkers Planet - " << nb_solved_levels << "/" << levels.size() << "\")\n"
      << "  (lives   " << player_status.lives << ")\n"
      << "  (score   " << player_status.score << ")\n"
      << "  (diamonds " << player_status.diamonds << ")\n"
      << "  (turr (x " << turr->get_tile_pos().x << ") (y " << turr->get_tile_pos().y << ")\n"
      << "       (back \"" << direction_to_string(turr->back_direction) << "\")\n"
...
WorldMap::loadgame(const std::string& filename)
{
  ...
  reader.read_int("diamonds", &player_status.diamonds);

  if (player_status.lives < 0)
    player_status.lives = START_LIVES;

  lisp_object_t* turr_cur = 0;
  if (reader.read_lisp("turr", &turr_cur))
    {
      Point p;
      std::string back_str = "none";
      std::string bonus_str = "none";

      LispReader turr_reader(turr_cur);
      turr_reader.read_int("x", &p.x);
      turr_reader.read_int("y", &p.y);
      turr_reader.read_string("back", &back_str);
      turr_reader.read_string("bonus", &bonus_str);
      
      player_status.bonus = string_to_bonus(bonus_str);
      turr->back_direction = string_to_direction(back_str);      
      turr->set_tile_pos(p);
    }
