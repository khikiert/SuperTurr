...
void draw_demo(GameSession* session, double frame_ratio)
{
  ...
  World::set_current(world);
		...
		//  turr->key_event((SDLKey) keymap.right,DOWN);
...
//      else {
//        logo5->draw( 160, 30);
//        turr->key_event((SDLKey) keymap.jump,DOWN); }
//    }
//  else
//    {
...
//    }
  ...
  // Wrap around at the end of the level back to the beginnig
		//  if(plevel->width * 32 - 320 < turr->base.x)
		//   {
		//      turr->level_begin();
		//      scroll_x = 0;
		//    }
		...
		//  turr->can_jump = true;
		//  float last_turr_x_pos = turr->base.x;
		//  world->action(frame_ratio);
  

  // disabled for now, since with the new jump code we easily get deadlocks
  // Jump if turr stays in the same position for one loop, ie. if he is
  // stuck behind a wall
		//  if (last_turr_x_pos == turr->base.x)
		//    {
		//      walking = false;
		//    }
...
/* --- TITLE SCREEN --- */
void draw_title(GameSession* session, double frame_ratio) {
  	global_frame_counter++;
	logo32->draw( 160, 30);
//	flipscreen();
}
...
void title(void)
{
//TestRotozoomSTurri* x = new TestRotozoomSTurri();
  random_timer.init(true);
//  logo_timer.init(true);
  ...
  /* Load images: */
  bkg_title = new Surface(datadir + "/images/background/superturr/title.png", USE_ALPHA);
  logo32 = new Surface(datadir + "/images/title/superturr/title-32.png", USE_ALPHA);
  ...
  while (Menu::current())
    {
      ...
      /* Lower the frame_ratio that Turri doesn't jump to hectically throught the demo. */
      frame_ratio = 0;
...
//    rysuje tytul
    if (Menu::current() == main_menu) draw_title(&session, frame_ratio);

	white_small_text->draw(" Super Turr :: ALPHA 19.12.31\n", 10, 460, 0);
      ...
      if(menu)
        {
          ...
          if(menu == main_menu)						// menu glowne na ekranie tytulowym
            {
              MusicManager* music_manager;
	      MusicRef menu_song;
              switch (main_menu->check())
                {
                case MNID_STARTGAME:				// rozpoczecie gry
                  // Start Game, ie. goto the slots menu
                  update_load_save_game_menu(load_game_menu);
                  break;
                case MNID_CONTRIB:					// bonus levels
                  // Contrib Menu
                  puts("Entering contrib menu");
                  generate_contrib_menu();
                  break;
                case MNID_LEVELEDITOR:				// edytor leveli
                  leveleditor();
                  Menu::set_current(main_menu);
                  break;
                case MNID_CREDITS: {				// credits
					...
					int Turri_test = 0;
					if (Turri_test) {
						char * png_file = "./data/images/background/superturr/caves.png";
						SDL_Surface * screen_SDL;
						SDL_RWops * rwop;
						rwop = SDL_RWFromFile(png_file, "rb");
						fprintf(stdout, "Creating surface from file: %s\n", png_file);
						fprintf(stdout, "title 1 \n");
						screen_SDL = IMG_LoadPNG_RW(rwop);
fprintf(stdout, "la");
SDL_Surface * x = new SDL_Surface;
x = Surface::CaptureScreen()->impl->get_sdl_surface();

//				screen_SDL = SDL_DisplayFormat(screen);
fprintf(stdout, "xa");
//				SDL_SetAlpha(screen_SDL, SDL_SWSURFACE, 0);
fprintf(stdout, "fa");
						fprintf(stdout, "title 2 \n");
						fade_surface(x, 15, "fadeout", 1, false);
						// fade_file("./data/images/background/superturr/gods.png", 15, "fadeout", 4, true);
						// fade_screen(15, "fadeout", 1);
						fprintf(stdout, "title 3 \n");
					}
					else {
						fade_file("./data/images/background/superturr/title.png", 15, "fadein", 1, false);
						display_text_file("CREDITS", bkg_title, SCROLL_SPEED_CREDITS);
					}

                  	music_manager->halt_music();
                  	menu_song = music_manager->load_music(datadir + "/music/superturr/title.ogg");
	                music_manager->play_music(menu_song,1);
                  	Menu::set_current(main_menu);
				  }
                  break;
                case MNID_QUITMAINMENU:				// wyjscie z gry
                  Menu::set_current(0);
                  break;
                }
            }
          else if(menu == options_menu)				// menu opcji
            {
              process_options_menu();
            }
          else if(menu == load_game_menu)			// menu po wybraniu "start game"
            {
              if(event.key.keysym.sym == SDLK_DELETE)
              ...
              else if (process_load_game_menu())
                {
                  ...
                  // reset turr
                  scroll_x = 0;
                  //titleturr.level_begin();
                  update_time = st_get_ticks();
                }
            }
          else if(menu == contrib_menu)				// menu po wybraniu "bonus levels"
            {
              check_contrib_menu();
            }
          else if (menu == contrib_subset_menu)		// jakies jeszcze inne menu...
            {
...
//  }
	...
	// wygaszanie screena przy wyjsciu z gry
