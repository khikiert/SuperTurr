...
Level::init_defaults()
{
  name       = "UnNamed";
  author     = "UnNamed";
  song_title = "superturr/turrican_on_a_beach.ogg";
  ...
  if (strcmp(lisp_symbol(lisp_car(root_obj)), "supertux-level") == 0)
    {
      ...
      reader.read_int("bkgd_blue_bottom",  &bkgd_bottom.blue);
		...
		if (!reader.read_int("music_loops", &music_loops)) {
			printf("Warning no music_loops specified for level.\n");
			music_loops = -1;
		}
